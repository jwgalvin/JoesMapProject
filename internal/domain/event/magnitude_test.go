...
func TestNewMagnitude(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			value:     5.5,
			scale:     MagnitudeScaleMw,
			wantValue: 5.5,
			wantScale: MagnitudeScaleMw,
			wantErr:   false,
		},
		{
		...
		},
		{
			...
			scale:     "invalid_scale",
			wantValue: 5.0,
			wantScale: MagnitudeScaleUnknown,
			wantErr:   false,
		},
		{
		...
		},
		{
			...
			scale:     "  ml  ",
			wantValue: 3.5,
			wantScale: MagnitudeScaleMl,
			wantErr:   false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, gotErr := NewMagnitude(tt.value, tt.scale)
			if gotErr != nil {
				if !tt.wantErr {
...
func TestMagnitude_String(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			value: 0.0,
			scale: MagnitudeScaleMl,
			want:  "0.0 ml",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m, err := NewMagnitude(tt.value, tt.scale)
			if err != nil {
...
func TestMagnitude_Value(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			value: -1.0,
			scale: MagnitudeScaleMb,
			want:  -1.0,
		},
		{
			name:  "Magnitude with maximum value",
			value: 10.0,
			scale: MagnitudeScaleMs,
			want:  10.0,
		},
	}
	for _, tt := range tests {
...
func TestMagnitude_Scale(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			value: 5.5,
			scale: MagnitudeScaleMw,
			want:  MagnitudeScaleMw,
		},
		{
			name:  "Magnitude with unknown scale",
			value: 4.0,
			scale: "invalid_scale",
			want:  MagnitudeScaleUnknown,
		},
		{
			name:  "Magnitude with another known scale",
			value: 3.0,
			scale: MagnitudeScaleMl,
			want:  MagnitudeScaleMl,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
